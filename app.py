...
def atualizar_status_agenda(id, novo_status):
    ...
    if compromisso:
        ...
        if novo_status in ['Pago', 'Concluído'] and status_anterior == 'A pagar' and valor > 0:
            ...
            descricao_financeiro = f"Faturamento ({novo_status}) - {compromisso['tipo_agendamento']} | Cli: {compromisso['nome_responsavel']} | Téc: {tecnico_nome} | Data: {compromisso['data_compromisso']} ({compromisso['hora_inicio']}-{compromisso['hora_fim']})"
            
            conn.execute('''
                INSERT INTO financeiro (tipo, categoria_fluxo, descricao, valor, data)
                VALUES (?, ?, ?, ?, ?)
            ''', ('entrada', 'Estúdios', descricao_financeiro, valor, datetime.now().strftime('%Y-%m-%d %H:%M')))
